...
class Dual_Path_Model(nn.Module):
    ...
    def __init__(
            ...
            max_length=20000,
            use_noise_ground=False,
            use_noise_ground_only=False
    ):
        ...
        self.use_noise_ground = use_noise_ground
        self.use_noise_ground_only = use_noise_ground_only

        self.conv2d = nn.Conv2d(
            out_channels, out_channels * (num_spks + 1), kernel_size=1
        ) if self.use_noise_ground and not self.use_noise_ground_only else nn.Conv2d(
    ...
    def forward(self, x):
        ...
        num_spks = self.num_spks + 1 if self.use_noise_ground and not self.use_noise_ground_only else self.num_spks

        # [B*spks, N, K, S]
        x = x.view(B * num_spks, -1, K, S)
