...
def build_spk_hashtable_librimix(hparams):
    ...
    for utt in tqdm(libri_utterances):
        ...
        # e.g. LibriSpeech/train-clean-360/441/128988/441-128988-0014.flac
...
def dynamic_mix_data_prep_librimix(hparams):
    ...
    def audio_pipeline(
        ...
        if hparams["use_wham_noise"]:
            ...
            start = 0
            stop = len_noise
            if len_noise > minlen:  # take a random window
                start = np.random.randint(0, len_noise - minlen)
                stop = start + minlen

            noise = noise[start:stop]
            # torchaudio.save(f"noise.wav", noise.unsqueeze(0), 8000)
            mixture = mixture[:min_len] + noise
            # torchaudio.save(f"mixture.wav", mixture.unsqueeze(0), 8000)
